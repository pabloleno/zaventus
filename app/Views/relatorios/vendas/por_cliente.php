                        ...
                        <div class="col-lg-4">
                            ...
                            <select class="form-control select2" name="id_cliente" style="width: 100%;">
                                <?php foreach ($clientes as $cliente) : ?>
                                    <?php $nome = ((int) $cliente['tipo'] === 1) ? $cliente['nome'] : $cliente['razao_social']; ?>
                                    <option value="<?= $cliente['id_cliente'] ?>" <?= (int) $cliente['id_cliente'] === (int) $id_cliente ? 'selected' : '' ?>><?= $nome ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-lg-3">
            ...
            <div class="card"><div class="card-body">
                ...
                <?php if (in_array($tipo_negocio, ['Todos', 'Produtos'], true)) : ?>
                    ...
                    <table class="table table-bordered table-striped">
                        <thead><tr><th>Cód.</th><th>Data</th><th>Hora</th><th>Valor</th><th>Caixa</th></tr></thead>
                        <tbody><?php if ($vendas) : foreach ($vendas as $venda) : ?><tr><td><?= $venda['id_venda'] ?></td><td><?= date('d/m/Y', strtotime($venda['data'])) ?></td><td><?= $venda['hora'] ?></td><td>R$ <?= number_format($venda['valor_a_pagar'], 2, ',', '.') ?></td><td><?= $venda['id_caixa'] ?></td></tr><?php endforeach; else : ?><tr><td colspan="5">Nenhuma venda de produto encontrada.</td></tr><?php endif; ?></tbody>
                    </table>
                <?php endif; ?>
                <?php if (in_array($tipo_negocio, ['Todos', 'Servicos'], true)) : ?>
                    <h6 style="margin-top: 30px"><b>Serviços</b></h6>
                    <table class="table table-bordered table-striped">
                        <thead><tr><th>OS</th><th>Saída</th><th>Hora</th><th>Valor</th><th>Situação</th></tr></thead>
                        <tbody><?php if ($ordens_servicos) : foreach ($ordens_servicos as $ordem) : ?><tr><td><?= $ordem['id_ordem'] ?></td><td><?= date('d/m/Y', strtotime($ordem['data_de_saida'])) ?></td><td><?= $ordem['hora_de_saida'] ?></td><td>R$ <?= number_format($ordem['valor_total'], 2, ',', '.') ?></td><td><?= $ordem['situacao'] ?></td></tr><?php endforeach; else : ?><tr><td colspan="5">Nenhuma venda de serviço encontrada.</td></tr><?php endif; ?></tbody>
                    </table>
                <?php endif; ?>
            </div></div>
        </div>
